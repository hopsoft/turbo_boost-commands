...
class DemosTest < ApplicationSystemTestCase
  test "turbo boost commands loaded and configured" do
    ...
    assert page.evaluate_script("Array.isArray(TurboBoost.Commands.eventDelegates)")
    assert page.evaluate_script("Array.isArray(TurboBoost.Commands.eventDelegates.find(e => e.name === 'change').selectors)")
    assert page.evaluate_script("Array.isArray(TurboBoost.Commands.eventDelegates.find(e => e.name === 'click').selectors)")
    assert page.evaluate_script("Array.isArray(TurboBoost.Commands.eventDelegates.find(e => e.name === 'submit').selectors)")
    assert_equal "function", page.evaluate_script("typeof TurboBoost.Commands.registerEventDelegate")
    assert_equal "object", page.evaluate_script("typeof TurboBoost.Commands.events")
